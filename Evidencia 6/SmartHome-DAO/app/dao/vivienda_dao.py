...
class ViviendaDAO(IViviendaDAO):
    ...
    def obtener_todos(self):
        conn = obtener_conexion()
        cursor = conn.cursor()
        cursor.execute("SELECT id_vivienda, nombre_vivienda, direccion, id_administrador, activa FROM Vivienda")
    ...
    def obtener_por_id(self, id_vivienda):
        conn = obtener_conexion()
        cursor = conn.cursor()
        cursor.execute("SELECT id_vivienda, nombre_vivienda, direccion, id_administrador, activa FROM Vivienda WHERE id_vivienda = %s", (id_vivienda,))
        row = cursor.fetchone()
        conn.close()
        if row:
    ...
    def actualizar(self, vivienda):
        ...
        cursor.execute(
            ...
            (vivienda.nombre_vivienda, vivienda.direccion, vivienda.activa, vivienda.id_administrador, vivienda.id_vivienda)
        )
        conn.commit()
        conn.close()
    ...
    def eliminar(self, id_vivienda):
        ...
        cursor.execute("DELETE FROM Usuario_Vivienda WHERE id_vivienda = %s", (id_vivienda,))
